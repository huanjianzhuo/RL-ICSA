# ===========================================================================
# Project:      ADRL - Adversarial Deep Reinforcement Learning
# File:         byzantine/attacks/minsum_attack.py
# Description:  MinSum攻击
# ===========================================================================

import torch
import numpy as np
import logging
from typing import List, Dict, Any, Tuple
from .base import CollusionAttackBase

logger = logging.getLogger(__name__)


class MinSumAttack(CollusionAttackBase):
    """
    AGR-agnostic Min-Sum 攻击（Attack-2）
    最小化距离和攻击：确保恶意梯度到所有良性梯度的距离和不超过良性梯度之间的最大距离和
    
    优化目标：
    argmax_γ Σ ||∇^m - ∇_i||_2^2 ≤ max_{i∈[n]} Σ_j ||∇_i - ∇_j||_2^2
    
    策略：
    1. 收集所有良性客户端的梯度
    2. 计算良性梯度的平均值 g_b
    3. 选择扰动方向 p = -g_b (反向攻击)
    4. 计算 S_max = max_k sum_j ||g_k - g_j||^2（良性客户端的最大距离和）
    5. 求解约束：sum_j ||(g_b + γp) - g_j||^2 ≤ S_max
    6. 求解二次方程得到 gamma
    7. 生成恶意梯度：∇^m = g_b + γ * p
    """
    
    def __init__(self, clients: List, config: Dict[str, Any], runner_instance):
        super().__init__(clients, config, runner_instance)
        self.safety_margin = config.get('minsum_safety_margin', 0.95)  # 安全边界
    
    def _get_params_vector(self, model: torch.nn.Module) -> torch.Tensor:
        """将模型参数展平为一维向量"""
        return torch.cat([p.data.view(-1) for p in model.parameters()])
    
    def _set_params_vector(self, model: torch.nn.Module, vec: torch.Tensor) -> None:
        """从一维向量恢复模型参数"""
        pointer = 0
        for p in model.parameters():
            n = p.numel()
            p.data = vec[pointer:pointer+n].view_as(p.data)
            pointer += n
    
    def _get_global_vector(self) -> torch.Tensor:
        """获取全局模型参数向量"""
        if hasattr(self.runner, 'server') and self.runner.server is not None:
            return torch.cat([p.data.view(-1) for p in self.runner.server.model.parameters()])
        return self._get_params_vector(self.clients[0].model)
    
    def _collect_benign_gradients(self) -> Tuple[torch.Tensor, torch.Tensor, list]:
        """
        收集良性客户端的梯度信息
        
        Returns:
            Tuple[torch.Tensor, torch.Tensor, list]: 
                - old_vec: 全局模型参数向量
                - g_b: 良性梯度平均值
                - grads: 良性梯度列表
        """
        old_vec = self._get_global_vector().detach().cpu()
        grads = []
        
        for c in self.benign_clients:
            if getattr(c, 'model', None) is None:
                continue
            w = self._get_params_vector(c.model).detach().cpu()
            grads.append(w - old_vec)
        
        if not grads:
            return old_vec, None, []
        
        G = torch.stack(grads, dim=0)
        g_b = G.mean(dim=0)
        
        return old_vec, g_b, grads
    
    def _generate_attack_params(self, client, agent_id: int) -> torch.Tensor:
        """
        生成 Min-Sum 攻击参数
        
        根据论文 Attack-2 (Min-Sum):
        目标: argmax_γ Σ_{i∈[n]} ||∇^m - ∇_i||_2^2 ≤ max_{i∈[n]} Σ_{j∈[n]} ||∇_i - ∇_j||_2^2
        生成: ∇^m = f_avg(∇_{i∈[n]}) + γ∇^p
        
        核心思路：
        1. 计算良性梯度平均值 f_avg(∇_{i∈[n]}) = g_b
        2. 选择扰动方向 ∇^p = -g_b（反向攻击，最大化破坏）
        3. 计算 S_max = max_k Σ_j ||∇_k - ∇_j||^2（良性客户端的最大距离和）
        4. 求解约束：Σ_j ||(g_b + γ∇^p) - ∇_j||^2 ≤ S_max
        5. 求解二次方程得到 gamma
        6. 生成恶意梯度: ∇^m = g_b + γ∇^p
        """
        old_vec, g_b, grads = self._collect_benign_gradients()
        
        if g_b is None:
            return None
        
        # 扰动方向 ∇^p：选择负的良性平均梯度（反向攻击）
        perturbation = -g_b
        p_norm2 = torch.dot(perturbation, perturbation).item() + 1e-12
        
        G = torch.stack(grads, dim=0)  # [m, d] 其中 m 是良性客户端数量
        
        # 计算 S_max = max_k Σ_j ||∇_k - ∇_j||^2
        # 这是良性客户端之间的最大距离和
        S_max = 0.0
        for k in range(G.size(0)):
            diff = G - G[k]  # [m, d]
            distance_sum = float((diff*diff).sum().item())
            S_max = max(S_max, distance_sum)
        
        # 求解二次约束：Σ_j ||(g_b + γ∇^p) - ∇_j||^2 ≤ S_max
        # 展开：Σ_j [||g_b - ∇_j||^2 + 2γ(g_b - ∇_j)·∇^p + γ^2||∇^p||^2] ≤ S_max
        # 整理：a*γ^2 + b*γ + c ≤ 0
        # 其中：
        #   a = m * ||∇^p||^2
        #   b = 2 * Σ_j (g_b - ∇_j)·∇^p
        #   c = Σ_j ||g_b - ∇_j||^2 - S_max
        
        gb_minus_G = (g_b.unsqueeze(0) - G)  # [m, d]
        
        # 二次方程系数
        a = G.size(0) * p_norm2  # m * ||∇^p||^2
        b = 2.0 * torch.sum(torch.mv(gb_minus_G, perturbation)).item()  # 2 * Σ_j (g_b - ∇_j)·∇^p
        c = float(torch.sum((gb_minus_G*gb_minus_G)).item() - S_max)  # Σ_j ||g_b - ∇_j||^2 - S_max
        
        # 求解判别式
        disc = b*b - 4*a*c
        if disc <= 0 or a <= 0:
            gamma = 0.0
        else:
            # 求两个根，选择较大的作为 gamma
            root1 = (-b - np.sqrt(disc)) / (2*a)
            root2 = (-b + np.sqrt(disc)) / (2*a)
            gamma = max(root1, root2)
            gamma = max(0.0, float(gamma))
        
        # 应用安全边界，避免数值问题
        gamma *= self.safety_margin
        
        # 生成恶意梯度: ∇^m = f_avg(∇_{i∈[n]}) + γ∇^p
        malicious_gradient = g_b + gamma * perturbation
        
        # 最终恶意参数: w^m = w_old + ∇^m
        crafted = old_vec + malicious_gradient
        
        return crafted



